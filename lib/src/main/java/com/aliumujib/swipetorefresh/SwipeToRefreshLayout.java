...
public class SwipeToRefreshLayout extends ViewGroup {
    ...
    public static final String TAG = "SwipeToRefreshLayout";
    ...
    private static final String LOG_TAG = SwipeToRefreshLayout.class.getSimpleName();
    ...
    private ProgressImage mCircleView;
    ...
    public SwipeToRefreshLayout(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.SwipeToRefreshLayout);
        RefreshDirection direction
                = RefreshDirection.getFromInt(a2.getInt(R.styleable.SwipeToRefreshLayout_srl_direction, 0));
        int drawableRes = a2.getResourceId(R.styleable.SwipeToRefreshLayout_srl_icon, 0);
        int animation = a2.getInt(R.styleable.SwipeToRefreshLayout_srl_animation, 0);
        if (direction != RefreshDirection.BOTH) {
            mDirection = direction;
            mBothDirection = false;
        } else {
            mDirection = RefreshDirection.TOP;
            mBothDirection = true;
        }
        a2.recycle();
        ...
        createProgressView(drawableRes, animation);
    ...
    private void createProgressView(@DrawableRes int loadingDrawable, int animation) {
        mCircleView = new ProgressImage(getContext(), CIRCLE_BG_LIGHT, CIRCLE_DIAMETER / 2);
        mProgress = new MaterialProgressDrawable(getContext(), this);
        mProgress.setBackgroundColor(CIRCLE_BG_LIGHT);
        if(loadingDrawable!=0){
            mCircleView.setImageResource(loadingDrawable);
        }else {
            mCircleView.setImageDrawable(mProgress);
        }
        mCircleView.setVisibility(View.GONE);
        addView(mCircleView);
        Log.d(TAG, String.valueOf(animation));
        mCircleView.attachAnimation(animation);
    }
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    if (y > mInitialDownY) {
                        setRawDirection(RefreshDirection.TOP);
                    } else if (y < mInitialDownY) {
                        setRawDirection(RefreshDirection.BOTTOM);
                    }
                    if ((mDirection == RefreshDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == RefreshDirection.TOP && canChildScrollUp())) {
                        mInitialDownY = y;
                        return false;
                    }
                }
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (mIsBeingDragged) {
                        ...
                        int targetY;
                        if (mDirection == RefreshDirection.TOP) {
...
//        }
    ...
    public interface OnRefreshListener {
        public void onRefresh(RefreshDirection direction);
    }

    public RefreshDirection getDirection() {
        return mBothDirection ? RefreshDirection.BOTH : mDirection;
    }

    public void setDirection(RefreshDirection direction) {
        if (direction == RefreshDirection.BOTH) {
            mBothDirection = true;
        } else {
            mBothDirection = false;
    ...
    // only TOP or Bottom
    private void setRawDirection(RefreshDirection direction) {
    ...
    public ProgressImage getCircleView() {
        return mCircleView;
    }
}
